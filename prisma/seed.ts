...
async function main() {
  ...
  const permissionsList: { id: string; chave: string }[] = [];
  ...
  for (const roleInfo of rolesData) {
    ...
    if (role.nome === 'VENDEDOR') {
      permsToAssign = permissionsList.filter(p =>
        ['dashboard', 'customers', 'sales', 'crm', 'agenda', 'reports'].includes(p.chave.split('.')[0]) &&
        !['delete', 'approve'].includes(p.chave.split('.')[1])
      );
    } else if (role.nome === 'FINANCEIRO') {
      permsToAssign = permissionsList.filter(p =>
        ['dashboard', 'finance', 'customers', 'suppliers', 'reports'].includes(p.chave.split('.')[0])
      );
    } else if (role.nome === 'ESTOQUISTA') {
      permsToAssign = permissionsList.filter(p =>
        ['dashboard', 'products', 'inventory', 'purchases', 'suppliers', 'reports'].includes(p.chave.split('.')[0])
      );
    }
